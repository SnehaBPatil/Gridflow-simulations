...
def emptypod(stop,src,final_pod,present_start_time):
    ...
    for i in range(0,len(path)-1):
        ...
        cursor.execute("SELECT * FROM journey WHERE src = ?  and start_time<=? ORDER BY start_time",(path[i],present_start_time-8))
        coming_to_stop = cursor.fetchall()
        present_start_time = collision(coming_to_stop, present_start_time, path[i],1)
        ...
        cursor.execute("SELECT * FROM emptypod WHERE src = ?  and start_time>=? ORDER BY start_time", (path[i], present_stop_time-8))
        ...
        cursor.execute("SELECT * FROM journey WHERE src = ?  and start_time>=?",(path[i],present_stop_time-8))
        coming_to_stop = cursor.fetchall()
        present_stop_time = collision(coming_to_stop, present_stop_time, path[i],1)
