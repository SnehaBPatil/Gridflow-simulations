...
def pick(passenger_id, arrival_time, src, dest, path):
    ...
    for stop in stops:
        ...
        if (pod != None):
            ...
            distance = shortestPath(stop, src)
    ...
    weight = weight / speed
    travel_time = shortestPath(src, dest) / speed
    if (stop != src):
...
def check_at_stop(stop):
    cursor.execute(
        ...
        (stop, stop, stop,
         stop))  # fetching data from pod table to find the earliest pod availability at given stop i.e stop
...
while (count != 0):
    ...
    cursor.execute("SELECT * FROM passenger  WHERE flag=? and departure_time=? ORDER BY arrival_time",
                   (0, 0))  # sorting passengers and serving based on arrival pod_available_time
